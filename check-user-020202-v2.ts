  ...
  if (user.athleteProfile?.raceGoals && user.athleteProfile.raceGoals.length > 0) {
    ...
    if (raceA) {
      ...
      if (!raceWeek) {
        ...
        console.log('  Plan range:', plan.startDate.toISOString().split('T')[0], 'to', plan.weeks[plan.weeks.length-1]?.endDate.toISOString().split('T')[0]);
      } else {
        ...
        if (!raceWorkout) {
          console.log('\n  ❌❌❌ BUG CONFIRMED: NO RACE WORKOUT IN RACE WEEK! ❌❌❌');
          console.log('  Expected: workout with type="race" on race day');
          console.log('  Actual: Long run or other workout instead');
        } else {
