...
async function applyMigration() {
  ...
  try {
    ...
    const verifyCustomWorkouts = await prisma.$queryRawUnsafe(`
    ...
    console.log('\n✅ custom_workouts:');
    verifyCustomWorkouts.forEach((col) => {
      console.log(`   - ${col.column_name} (${col.data_type})`);
    });

    const verifyAthleteProfiles = await prisma.$queryRawUnsafe(`
    ...
    console.log('\n✅ athlete_profiles:');
    verifyAthleteProfiles.forEach((col) => {
      console.log(`   - ${col.column_name} (${col.data_type})`);
    });
