...
// These will be moved inside component for i18n
...
export default function OnboardingPage() {
  ...
  const handleGoBack = () => {
    // V1.3.0: Navegação simplificada para 3 fases
    if (step === 3 && formData.hasRunBefore === 'no') {
      // Se nunca correu e está nos objetivos, volta para perfil
      setStep(1);
    } else if (step > 1) {
      // Volta um step
      setStep(step - 1);
          ...
          <CardHeader>
            <div className="flex items-center justify-between mb-2">
              <Badge variant="outline" className="text-sm">
                Fase {step > 1 && formData.hasRunBefore === 'no' ? step - 1 : step} de {formData.hasRunBefore === 'no' ? '2' : '3'}
              </Badge>
            </div>
            <CardTitle className="text-3xl">
              {step === 1 && '🏃 Fase 1: Seu Perfil de Corredor'}
              {step === 2 && '📊 Desempenho Atual'}
              {step === 3 && '🎯 Fase 2: Objetivos e Metas'}
              {step === 4 && '📅 Fase 3: Sua Disponibilidade'}
            </CardTitle>
            <CardDescription>
              {step === 1 && 'Conte-nos sobre você e sua experiência com corrida'}
              {step === 2 && 'Seus ritmos habituais nos ajudam a personalizar melhor o plano'}
              {step === 3 && 'Defina sua corrida alvo e suas metas'}
              {step === 4 && 'Configure os dias e modalidades que você pode treinar'}
            </CardDescription>
          </CardHeader>
              ...
              {step === 1 && (
                <>
                  <Alert className="border-blue-300 bg-blue-50">
                    ...
                    <AlertDescription>
                      <strong>🧠 IA Personalizada:</strong> Todas as informações que você fornecer serão analisadas pela nossa IA para criar um plano verdadeiramente personalizado para você.
                    </AlertDescription>
                  </Alert>
