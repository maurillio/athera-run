...
CREATE TABLE IF NOT EXISTS user_decision_patterns (
  ...
  CONSTRAINT valid_scores CHECK (
    date_score IS NULL OR (date_score >= 0 AND date_score <= 100) AND
    type_score IS NULL OR (type_score >= 0 AND type_score <= 100) AND
    volume_score IS NULL OR (volume_score >= 0 AND volume_score <= 100) AND
    intensity_score IS NULL OR (intensity_score >= 0 AND intensity_score <= 100)
  )
);
...
-- Ver estatísticas de decisões por usuário
-- SELECT 
--   user_id,
--   decision_type,
--   COUNT(*) as total,
--   ROUND(AVG(confidence_at_decision), 2) as avg_confidence,
--   ROUND(AVG(date_score), 2) as avg_date_score,
--   ROUND(AVG(type_score), 2) as avg_type_score
-- FROM user_decision_patterns
-- GROUP BY user_id, decision_type
-- ORDER BY user_id, decision_type;

-- Ver calibrações recentes
-- SELECT 
--   user_id,
--   current_threshold,
...
--   optimization_trigger
-- FROM user_score_weights
-- ORDER BY decisions_analyzed DESC
-- LIMIT 10;
